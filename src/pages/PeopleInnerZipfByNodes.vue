 ...
 * @LastEditors: ider
 * @LastEditTime: 2020-09-04 15:36:29
 * @Description:
 -->
...
const LC = new localCache({
  storeName: 'peopleinnerzipfbynodes', // Should be alphanumeric, with underscores.
...
export default {
  ...
  methods: {
    async handleSlopeTrend(subject) {
      ...
      if (!item) {
        try {
          const res = await getPeopleInnerZipfByNodes(opt)
          if (res.data) {
            const gradientList = []
    ...
    async calZipf() {
      ...
      await getPeopleInnerZipfByNodes(opt)
        .then(res => {
          if (res.data) {
    ...
    setOptions_slope(dataList) {
      ...
      for (const data of dataList) {
        ...
        const title = data.title.replace(' zipf分布', '')
