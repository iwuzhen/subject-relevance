...
v-container(fluid)
  v-row
    v-col(cols='11')
      v-select(v-model='subjectSelect' :items='subjectOpt' small-chips multiple deletable-chips clearable label='目标学科' @change='getData')
    v-col(cols='4')
      v-select(v-model='typeSelect' :items='typeOpt' label='世界类型' @change='getData')
    v-col(cols='4')
      v-select(v-model='isNoRefSelect' :items='isNoRefOpt' label='过滤' @change='getData')
  v-row
    v-col(cols='11')
...
export default {
  ...
  data() {
    return {
      ...
      isNoRefSelect: 0,
      isNoRefOpt: [{ text: '保留引用为 0 的文章', value: 0 }, { text: '去掉引用为 0 的文章', value: 1 }],
      loading: false,
      chartOpt: {},
      chartData: {},
      gridData: [],
  ...
  methods: {
    ...
    getData: _.debounce(async function() {
      ...
      let opt = {
        ...
        isNoRef: this.isNoRefSelect,
        islog: 1,
        x_from: this.nodeRange[0],
        x_to: this.nodeRange[1],
