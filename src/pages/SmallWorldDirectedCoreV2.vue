 ...
 * @LastEditors: ider
 * @LastEditTime: 2020-09-04 15:57:42
 * @Description:
 -->
...
export default {
  name: 'CoreV2SmallWorld',
  extends: Base,
  data() {
    return {
      pageName: 'CoreV2 SmallWorld 有向图学科间趋势',
      dialog: false,
      loading: false,
      currentSubjectSelect: '',
      targetSubjectSelect: [],
      methodSelect: 'average_distance',
      graphSizeSelect: 3000,
      graphSizeOpt: [
        {
          value: 999999,
          text: '最大节点数'
  ...
  methods: {
    ...
    async getData() {
      ...
      let allYear = []
      const subjectData = []
      for (const subjectName of this.targetSubjectSelect) {
        const repdata = await this.getOneDate(subjectName)
        console.log(repdata)
      ...
      const options = this.setOptions(ret_data)
      console.log(this.myChartObjs)
      ...
      const _opt = {
        ...
        tooltip: {
          ...
          formatter: function(params) {
            ...
            for (const row of zhenxiang) {
              showHtm += `${row[0]} ${row[1]}：${row[2]}<br>`
            }
            showHtm += '目标学科到当前学科<br>'
            for (const row of nixiang) {
              showHtm += `${row[0]} ${row[1]}：${row[2]}<br>`
            }
            showHtm += '差值图<br>'
            for (const row of chazhi) {
              showHtm += `${row[0]} ${row[1]}：${row[2]}<br>`
            }
            return showHtm
          }
        ...
        series: (data => {
          const ee = data.b.map((item, index) => {
