 ...
 * @LastEditors: ider
 * @LastEditTime: 2020-06-06 12:36:08
 * @Description: 
 -->
...
<script>
import { getCoreZipfByNodes_v2 } from "@/api/index";
...
export default {
  ...
  methods: {
    async handleSlopeTrend(subject) {
      ...
      if (!item) {
        try {
          let res = await getCoreZipfByNodes_v2(opt);
          if (res.data) {
            let gradientList = [];
    ...
    async calZipf() {
      ...
      await getCoreZipfByNodes_v2(opt)
        .then(res => {
          if (res.data) {
